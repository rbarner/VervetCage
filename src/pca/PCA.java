...
public class PCA
{
	...
	{
		File f = new File("/Users/rbarner/Dropbox/FodorLab/KylieData/Analysis/FunctionPrediction/vervet/pcas/pcoaTables/_dateSeparated/jan12/jan12PhylumLevelNormPivot.txt");
		List<String> categoryHeaders = new ArrayList<String>();
		List<List<String>> categories = new ArrayList<List<String>>();
		BufferedReader reader = new BufferedReader(new FileReader("/Users/rbarner/Dropbox/FodorLab/KylieData/rawData/sixteenSseqs/transposed/orderedPivots/processedPivotTables/phylum/jan12PhylumLevelLoggedPivot.txt"));
		...
		DistanceMeasureInterface dmi = new SquareMatrix();
		double [][] d = new double[14][10];
		int x = 0;
		while(nextLine != null)
		{
			...
			{
				d[x][y] = Double.parseDouble(sToken.nextToken());
				y++;
			}
			x++;
		...
		double [][] transposedD = transposeArray(d);
		
		writePCAFile(keys, categoryHeaders
				, categories, transposedD,f, dmi); 
	}
}
