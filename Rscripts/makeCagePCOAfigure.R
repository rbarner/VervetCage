...
#Split sample ID by date
out=split(sampleData, f=sampleData$Date)

#Make a separate data frame for each of the dates
...
#Read in PCOA information (.axes is the file with the eigenvectors, .loadings is the percent of variance captured by each of the coordinates)
#PCA Genus is all the samples from all the dates
...
#Create a function that will make a pdf that is made of a pcoa plot of all samples colored by cage and a plot for each of the dates colored by cage
...
{
	#Create x and y axis lables for all of the plots
